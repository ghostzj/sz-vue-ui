<template>
  <div
    ...
    <sz-input
      ...
      <template slot="suffix">
        <i
          ...
          key="clear"
          class="sz-input__icon sz-icon-circle-close"
    ...
    <div v-if="multiple" class="sz-cascader__tags">
      <sz-tag
      ...
      <input
        ...
        type="text"
        class="sz-cascader__search-input"
    ...
    <transition name="sz-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        v-show="dropDownVisible"
        ref="popper"
        :class="['sz-popper', 'sz-cascader__dropdown', popperClass]">
        <sz-cascader-panel
          ref="panel"
          v-show="!filtering"
          ...
          @close="toggleDropDownVisible(false)"></sz-cascader-panel>
        <sz-scrollbar
          ref="suggestionPanel"
          v-if="filterable"
          v-show="filtering"
          tag="ul"
          class="sz-cascader__suggestion-panel"
          view-class="sz-cascader__suggestion-list"
          ...
          <template v-if="suggestions.length">
            <li
              ...
              :key="item.uid"
              :class="[
                'sz-cascader__suggestion-item',
                item.checked && 'is-checked'
              ]"
              :tabindex="-1"
              @click="handleSuggestionClick(index)">
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="sz-icon-check"></i>
            </li>
          </template>
          <slot v-else name="empty">
            <li class="sz-cascader__empty-text">{{ t('el.cascader.noMatch') }}</li>
          </slot>
        </sz-scrollbar>
      </div>
    </transition>
...
import SzCascaderPanel from '../../cascader-panel';
...
export default {
  ...
  props: {
    ...
    placeholder: {
      type: String,
      default: () => t('sz.cascader.placeholder')
    },
    disabled: Boolean,
    clearable: Boolean,
    filterable: Boolean,
  ...
  computed: {
    realSize() {
      const _szFormItemSize = (this.szFormItem || {}).szFormItemSize;
      return this.size || _szFormItemSize || (this.$ELEMENT || {}).size;
    },
    tagSize() {
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.sz-cascader__suggestion-item');
        } else {
          const firstMenu = popper.querySelector('.sz-cascader-menu');
          firstNode = firstMenu.querySelector('.sz-cascader-node[tabindex="-1"]');
        }
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.sz-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.sz-cascader__tags');
      let suggestionPanelEl = null;

      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector('.sz-cascader__suggestion-list');
