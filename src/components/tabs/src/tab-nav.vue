...
export default {
  ...
  render(h) {
    ...
    const scrollBtn = scrollable
      ? [
        <span class={['sz-tabs__nav-prev', scrollable.prev ? '' : 'is-disabled']} on-click={scrollPrev}><i class="sz-icon-arrow-left"></i></span>,
        <span class={['sz-tabs__nav-next', scrollable.next ? '' : 'is-disabled']} on-click={scrollNext}><i class="sz-icon-arrow-right"></i></span>
      ] : null;
    ...
    const tabs = this._l(panes, (pane, index) => {
      ...
      const btnClose = closable
        ? <span class="sz-icon-close" on-click={(ev) => { onTabRemove(pane, ev); }}></span>
        : null;
    ...
    return (
      <div class={['sz-tabs__nav-wrap', scrollable ? 'is-scrollable' : '', `is-${this.rootTabs.tabPosition}`]}>
        {scrollBtn}
        <div class={['sz-tabs__nav-scroll']} ref="navScroll">
          <div
            class={['sz-tabs__nav', `is-${this.rootTabs.tabPosition}`, stretch && ['top', 'bottom'].indexOf(this.rootTabs.tabPosition) !== -1 ? 'is-stretch' : '']}
            ref="nav"
            style={navStyle}
            role="tablist"
